...
0x0803f7ff   254k   0x0803f000
0x0803ffff   256k   0x0803f800
*/
#define base_addr0 0x0803f000
#define base_addr1 0x0803f800
#define sector_size 0x0800   //2k
u16 flash_ee_data_read(u16 address, u16* pdata);
/**
  **************************************************************************
  * @file     eeprom.c
  * @version  v2.0.0
...
#define EE_VALID_PAGE0                  ((u16)0x0000)  /*!< the effective page is page 0 */ 
#define EE_VALID_PAGE1                  ((u16)0x0001)  /*!< the effective page is page 1 */ 
#define EE_VALID_PAGE_NONE              ((u16)0x0002)  /*!< no valid page found */
                                                              
#define EE_PAGE_ERASED                  ((u16)0xFFFF)  /*!< page is in erased state */ 
#define EE_PAGE_TRANSFER                ((u16)0xCCCC)  /*!< page is in transfer state */ 
#define EE_PAGE_VALID                   ((u16)0x0000)  /*!< page is in valid state */ 
...
/** 
  ...
  */
u16 flash_ee_valid_page_get(ee_valid_page_type mode)
{
  u16 page0_status;
  u16 page1_status;

  /* get page 0 status */ 
  page0_status = (*(__IO u16*)EE_PAGE0_ADDRESS);

  /* get page 1 status */ 
  page1_status = (*(__IO u16*)EE_PAGE1_ADDRESS);
...
/** 
  ...
  * @retval flash_status.
  */
flash_status_type flash_ee_write_no_check(u16 address, u16 data)
{
  u16 valid_page;
...
/** 
  ...
  */
u16 flash_ee_format_check(u16 page0_status, u16 page1_status)
{
...
/** 
  ...
  *         - 1: the format is incorrect
  */
flash_status_type flash_ee_erase_transfer(u16 page0_status, u16 page1_status)
{
  if (page0_status == EE_PAGE_TRANSFER)
  {
    /* mark the status of page 0 as VALID */
...
/** 
  ...
  *         - 1: the format is incorrect
  */
flash_status_type flash_ee_valid_transfer(u16 page0_status, u16 page1_status)
{                                  
...
/** 
  ...
  * @retval flash_status.
  */
flash_status_type flash_ee_data_write(u16 address, u16 data)
{
  flash_status_type flash_status = FLASH_OPERATE_DONE;
...
u16 flash_ee_data_read(u16 address, u16* pdata)
{
  ...
  while (find_address > start_address)
  {
    ...
    data_address = (*(__IO u16*)find_address);

    /* variable address matching */ 
    if (address == data_address)
    {
      /* read data */ 
      *pdata = (*(__IO u16*)(find_address - 2));
...
u16 read_ee(u16 *p,u16 len)
{
	...
	if(stat==0)
	{
		stat=1;
		flash_ee_init();
		if(flash_ee_init()!=FLASH_OPERATE_DONE)return 0xfffe;
	}
	
	  for(i = 0; i < len; i++)
  {
   
    ret=flash_ee_data_read(i, (p+i));
    if( ret!=0)return ret;
  }
	return 0;
}

u16 write_ee(u16 *p,u16 len)
{
	...
	for(i = 0; i < len; i++)
  {
    ...
    flash_ee_data_write(i,*(p+i));
  }
	
	if(read_ee(a,len)!=0)return 1;
	for(i = 0; i < len; i++)
  {
		  if(a[i]!=*(p+i))return 1;
